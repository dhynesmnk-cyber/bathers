...
**2026-09-17 revision (owner decision) — the chooser moves above the foreword, and springs get their own entry.** The 2026-07-23 exception above places the chooser "between the foreword and the results area". It now sits directly under the masthead's subtitle lines, with the foreword below it and the results area still last. The reason is measurable rather than editorial: with the hero masthead (§5c, §7) taking the first ~350px and two paragraphs of foreword after it, the first control a visitor could act on began around 1060px down, below the fold on a laptop. It now begins around 675px. The foreword keeps its full editorial weight and its "choose by vibes" close; it is read after the page has shown what it can do, not before. Nothing else about the exception changes: the chooser stays inside the reading-spine rhythm, still fronts the existing filter and place pages, and is still not a hero banner.

Within the chooser, a **springs block** now leads the browse routes, above the by-pool-setting/by-place row: a mono label, one line of prose, and the two springs routes that already exist (`/natural-spring/` and `/magnesium-pool/`). It is marked out by space alone, with no rule, border or fill of its own — `.finder-row` already closes with a hairline immediately above it, and two hairlines 2.5rem apart read as a mistake rather than a hierarchy. Springs are removed from the "By pool setting" column while this block is present, since the same link 90px away reads as a bug rather than as emphasis; when no spring venue is published the block does not render and the column carries the full list again. Both routes stay present-only like everything else in the chooser.
