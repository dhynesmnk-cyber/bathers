...
**2026-09-17 revision — chooser raised above the foreword; springs promoted (owner decision).** Content order is now masthead → chooser section → editorial foreword → results area. The foreword and the results area are otherwise unchanged, and **the results area stays empty by default** — the 2026-07-26 removal of the default contents list below was considered again in the same conversation and deliberately kept. The chooser gains a springs block ahead of its browse columns (DESIGN.md §5, 2026-09-17), and the guidance line now names what each of the three browse routes is for, which is the thing the previous wording left the visitor to infer. The foreword was rewritten to lead with springs rather than with the first minutes in a sauna; its "choose by vibes" close is kept, and the paragraph that restated the chooser's own guidance is gone.
