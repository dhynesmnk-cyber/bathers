...
- **Chooser section** *(2026-07-23, extended 2026-07-26)*: one short line of plain-prose usage guidance, a search field, a "near you" field, then two ways into the directory. State links (plain mono text) navigate straight to `/{state}/`. Amenity triggers (large icon+label, DESIGN.md §6) are plain anchor links to `/?amenities={key}#results-heading` — the same `amenities` URL query parameter the inline filter bar already reads on load — so clicking one lands back on the index pre-filtered to that amenity, scrolled to the results area. All of these are normal links, so they degrade correctly without JavaScript. The pool-type grouping (thermal springs/indoor/outdoor/other) that the former default contents list sorted by has no homepage entry point of its own — it stays reachable via each state's `/[state]/[pooltype]/` page (§2.3, unchanged) and is otherwise surfaced per-venue via promoted feature badges (DESIGN.md §6/Features.astro).
- **Search** *(2026-07-25, TRD.md §8 exception, unchanged 2026-07-26)*: a text field (homepage chooser and corner menu) matches against name/suburb/state/category/amenities and lists up to 8 results as a live dropdown directly beneath the input — plain typographic links to `/spa/[slug]/` — keyboard-navigable (↑/↓/Enter/Escape). This is the homepage's "search first, then venues appear" behaviour; there is no separate full-width search-results list. Requires JavaScript — degrades to a `<noscript>` line pointing back to state/amenity/setting browsing.
- **Results area** *(2026-07-26, replaces the former map)*: sits below the chooser. Empty by default, showing a direction line (`Pick an amenity above, or tell us where you are, to see venues here.`). Populated by the amenity filter toggles and/or a near-me lookup (below) — not by search, which keeps its own dropdown, above. Each match renders as a typographic `VenueEntry` row — same "no cards" list treatment the former contents list used (DESIGN.md §5).
- Amenity filter toggles (inline text, per DESIGN.md §5) continue the pre-2026-07-26 amenity-chooser-icon behaviour: with no near-me lookup active, activating a filter reveals every published venue matching it, sitewide (this is what the homepage chooser's amenity triggers link to, unchanged target `#results-heading`). With a near-me lookup active, the filter narrows that list instead. Filter state syncs to the URL query string so filtered views are shareable/bookmarkable.
- **Near me** *(2026-07-25, revised 2026-07-26, TRD.md §8 exception)*: a manual suburb/postcode field (never browser geolocation) resolves against a hand-curated place gazetteer, then filters — not re-sorts — to venues within a straight-line **50km radius**, sorted nearest-first, into the results area above. No map, so no "you are here" marker. Composes with the amenity filter above — the radius list only includes venues currently passing that filter. State syncs to `?near=`, cleared via a `Clear` control. An empty radius (no venue within 50km) shows its own direction line (`No venues within 50km of {place}.`). Requires JavaScript — degrades to a `<noscript>` line; the plain by-state/by-amenity/by-setting browsing underneath is unaffected.
- Everything except search and near-me works with JavaScript disabled (the by-state/by-amenity/by-setting chooser links and the programmatic pages they lead to are plain server-rendered links).
