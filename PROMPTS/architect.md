...
- `price` (structured, 2026-07-31): a `price` object with `adult_drop_in` and/or `standard_session` (plain numbers, in the venue's own currency), alongside — never instead of — the freeform `cost` string. Set `adult_drop_in` only when `facts.pricing` states a real general-admission bathing price (the cheapest way one adult gets into the water); set `standard_session` for the venue's typical/standard session where distinct. Every number here must also appear in the `cost` string (they're cross-checked). **Omit `price` entirely** for package-only or treatment-led venues with no drop-in bathing price — do not derive a number from a treatment or package rate, and never invent one. `verification` and `drive_time` are stamped by the pipeline, not you — don't write them.
...
- `facilities`: set a key `true` only on explicit evidence in `facts` (e.g. an explicit mention of on-site parking, towels supplied, changerooms, booking requirement, wheelchair access, an outdoor/open-air pool, an indoor pool, a natural/geothermal spring source, step-free entry, a hoist, or accessible changerooms). Unmentioned facilities are omitted from the object rather than guessed `false`; the reviewer fills gaps by hand, same posture as amenities. Note `pregnancy_safe` is the one exception — never set it yourself (SCHEMA.md §1a, manual reviewer-set only).
- `temperatures` (2026-07-26): set `sauna_min_c`/`sauna_max_c` (equal for a single stated figure) or `cold_plunge_min_c`/`cold_plunge_max_c` only when `facts` states a genuine single temperature or one continuous range for that circuit. If a venue has more than one sauna, plunge, or heat room at materially different stated temperatures, use `sauna_display`/`cold_plunge_display` instead (a short string like `"Rock sauna 99°C, wood-fired sauna 85°C"`) rather than collapsing them into a range that would misrepresent the venue. Omit whichever keys, or the whole object, that `facts` doesn't support. Never infer a number from a venue's category or general reputation.
- `dress_code`, `session_gender`, `session_gender_note`, `silence_policy`, `phone_policy`, `minimum_age` (2026-07-26): same rule as everywhere else in this section, set only on explicit evidence in `facts` (dress code and session-gender/age/policy details typically turn up in `facts.setting` or `facts.other` when a venue's own site states them), omitted otherwise. These are the structured, filterable-in-future form of facts that already have a home in the FAQ (see below) — draft both when the facts support it; the frontmatter field and the FAQ mention are not a duplication to avoid.
