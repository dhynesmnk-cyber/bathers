...
- `price` (structured, 2026-07-31): a `price` object with `adult_drop_in_aud` and/or `standard_session_aud` (plain numbers, AUD), alongside — never instead of — the freeform `cost` string. Set `adult_drop_in_aud` only when `facts.pricing` states a real general-admission bathing price (the cheapest way one adult gets into the water); set `standard_session_aud` for the venue's typical/standard session where distinct. Every number here must also appear in the `cost` string (they're cross-checked). **Omit `price` entirely** for package-only or treatment-led venues with no drop-in bathing price — do not derive a number from a treatment or package rate, and never invent one. `verification` and `drive_time` are stamped by the pipeline, not you — don't write them.
