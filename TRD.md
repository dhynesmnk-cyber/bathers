...
**RESOLVED 2026-09-08 — country-namespaced URLs.** Geography now lives under `/places/<world-region>/<country>/<subdivision>/`, with a fourth level for areas and per-subdivision feature filters. Slugs are full names (`western-australia`, `washington`), so nothing collides and a URL reads as itself. The `/places/` prefix is structural, not decorative: without it a country page would sit at the same route depth as a state filter page and Astro could not disambiguate two dynamic routes at one depth. Every old geographic URL 301s to its new home (`netlify.toml`), the sitemap and `llms.txt` are regenerated from the same tables the routes build from, and `admin/pipeline/validate_places.py` asserts the hierarchy links every level to the one below and that area slugs never collide with filter slugs in the shared leaf slot. The region taxonomy stays Australian and its lookup still refuses non-AU venues. The original problem statement follows, for the record.

*(Superseded problem statement.)* **Open decision, due before the first non-AU venue is published: country-namespaced URLs.** Subdivision pages are currently `/<subdivision>/` (`/vic/`, `/nsw/`), which is unambiguous while every venue is Australian and breaks the moment one is not — `WA` is Western Australia under AU and Washington under US, and they would collide on `/wa/`. The region taxonomy (`site/src/data/regions.ts`) is Australian-only for the same reason and its lookup now refuses non-AU venues outright rather than mismatching them. Resolving this means changing live, indexed URLs, which is why it is not being decided incidentally here: Gates 6–11 were an SEO engagement and every one of those routes is in `sitemap.xml`. Publishing a US venue before this is settled would produce a wrong or colliding page.
...
**Exception (user-approved, 2026-07-31) — OSRM public routing demo for drive-time (no new package).** CLAUDE.md rule 2 applies; this is that ask, approved. Gate 7's `drive_time` field is computed once per venue, at harvest/backfill time, from the venue's coordinates to its nearest state capital, via OSRM's public routing demo (`router.project-osrm.org`) called directly over the already-installed `httpx` — **no new dependency**, cached on disk (`admin/pipeline/drivetime.py`), recomputed only when coordinates change. This mirrors the project's accepted posture for Nominatim geocoding (`GEOCODER=nominatim`): a public OSM-ecosystem tier, no API key, $0. *Rejected alternatives:* Google Distance Matrix (metered, requires Cloud billing, buys traffic-aware accuracy this batch use doesn't need); a haversine × speed factor (the existing `haversineDistanceKm` would misrepresent drive time for exactly the remote, winding-road venues this directory covers). *Caveat and fallback:* OSRM's own docs discourage production reliance on the public demo; given batch, infrequent use (25 venues, a handful per coverage addition) that's an acceptable starting risk. If it becomes unreliable, the named fallback is a self-hosted OSRM + AU OSM extract run as a one-off batch job — not built until needed.

**Hosting posture correction (Gate 12, 2026-09-08).** The stack table above says the admin app "Runs on localhost only." That has not been true since 2026-07-21, when it was deployed to Fly.io (`bathers-admin`, syd, `fly.toml`) so the claim form's cross-origin POST and Stripe's webhook could reach it. This entry records the real posture rather than leaving the table wrong: **the admin app is reachable from the public internet.**

Everything in Gate 12 follows from that one fact. The app holds `ANTHROPIC_API_KEY`, `STRIPE_SECRET_KEY`, `NETLIFY_AUTH_TOKEN`, `SMTP_PASSWORD`, `GOOGLE_PLACES_API_KEY` and `GITHUB_PAT`, and it is the only component in this system that commits and pushes to git — so a compromise means arbitrary published content, spend on three metered APIs, and the requester PII in `claims.db`. Until Gate 12 the auth on all of that failed *open*: blank `ADMIN_USERNAME`/`ADMIN_PASSWORD` disabled the check entirely, silently. It now fails closed at import (`admin/security.py`) and at boot (`docker-entrypoint.sh`).
...
- **Venue type — generic `LocalBusiness`, deliberately retained.** *Against* a per-category switch to `DaySpa`/`HealthAndBeautyBusiness`: it would re-introduce exactly the "these aren't day spas" mislabel the 2026-07-26 category decision rejected (`day_spa` retired in favour of `hotel_spa`). Specificity is carried by an `amenityFeature` array (`LocationFeatureSpecification` per amenity/facility) on the `LocalBusiness`, not by a narrower type. Emitted from `site/src/pages/spa/[slug].astro`.
- **Blog `author` — a named byline field added.** An optional `author` string on the blog content collection (`site/src/content/config.ts`); when a post sets it, `BlogPosting.author` becomes a `Person`, otherwise it stays the organisation. Never a fabricated name — posts without a real author supplied stay org-authored. The field ships now; the author name(s) to populate it are pending from the site owner.
...
**Exception (user-approved, 2026-08-03) — operator force-publish override on the article approve gate.** The `/articles` review screen normally refuses to publish an article/essay whose fact-check report is `blocked` (unsupported claims). A deliberate operator action ("Force publish (override block)", offered only when the report is blocked) may now publish past that block for claims the human has verified by hand. Scope and honesty (CLAUDE.md rule 6) are fixed: the override bypasses **only** the fact-check block — never the structural checks in `articles._validate_for_publish` (title/summary, no hardcoded figures, house register, ≥5-venue eligibility) and never a missing report — and it never fakes a clean check. The operator's reason is stamped onto the committed fact-check report as an `override: {reason, at}` record; the unsupported claims and counts stay visible in that record. `validate_articles.check_articles` honours the `override` marker so the build gate accepts the published article while the record still shows what was overridden and why. See `admin/pipeline/articles.approve(slug, override_reason=…)`, `admin/pipeline/validate_articles.py`, and the `/articles` screen's force-publish action. Separately, that screen also gains a manual "Post to live" deploy control (streams `POST /api/deploy`) so a publish that didn't reach the live site — an auto-deploy failure is server-log-only — can be re-run visibly.
