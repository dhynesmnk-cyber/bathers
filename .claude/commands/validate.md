...
28. **Coverage floor & concentration ceiling.** `python3 -m admin.pipeline.validate_coverage` must exit 0 and prints the authoritative gap: how far each claimed subdivision is from the floor of 5, and how many venues elsewhere the 40% concentration ceiling needs. **The floor and ceiling report rather than fail while Gate 14 is open** — a content backlog held red for as long as it takes to clear is a suite nobody reads. What fails today is a malformed entry in `data/coverage-reasons.json`, the gate contract's "logged, dated reason" escape: an undated or placeholder reason is the hatch being used without saying why. Only AU subdivisions and non-AU ones that already have a venue are held to the floor, so 51 empty US states are not noise. Closing Gate 14 means flipping `FLOOR_IS_BLOCKING` to True. `--self-test` proves the reasons validation and the ceiling arithmetic.
